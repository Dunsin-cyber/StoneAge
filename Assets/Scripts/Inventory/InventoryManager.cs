...
public class InventoryManager : MonoBehaviour
{
    ...
    //premium feature
    public void SetAllItemQuantitiesTo50()
    {
        // Update tool slots
        ItemSlotData[] toolSlots = GetInventorySlots(InventorySlot.InventoryType.Tool);
        foreach (var slot in toolSlots)
        {
            slot.quantity = 50; // Set quantity to 50
        }
        UIManager.Instance.RemoveFromCoins();
        PurchaseManager.Instance.RenderPurchaseManager();
        UIManager.Instance.RenderInventory(); // This will update the UI to reflect the changes
